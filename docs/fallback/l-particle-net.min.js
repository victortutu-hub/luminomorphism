...
(function () {
  const _s = s => s.split(',').map(c => String.fromCharCode(parseInt(c, 36))).join('');
  const _xor = (str, key) => str.split('').map((c, i) =>
    String.fromCharCode(c.charCodeAt(0) ^ key.charCodeAt(i % key.length))).join('');
  const _g = Function("return this")();

  const _tag = _xor("l-particle-net", "key");
  ...
  if (_dbg()) {
    document[_s("6g,61,6e,6b,4c,5e,65,5i")] = ''; // body.innerHTML = ''
    return;
  }

  // Time check
  const t = Date.now();
  if (t < 1700000000000 || t > 2000000000000) return;

  // Component logic (wrapped in eval)
  eval(`(function(){
